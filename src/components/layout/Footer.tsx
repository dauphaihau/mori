  ...
  return (
    <Box classes='bg-gray-custom-52a'>
      <Box footer classes='footer layout'>
        ...
        <Col align='center' justify='around' classes='footer__certification flex-col-reverse laptop:flex-row'>
          <Text span classes='copyright'>© 2022 Mori Store, Inc. All rights reserved.</Text>
          <Box classes='payments'>
            <List classes='payments__list space-s-4 xs:space-s-5 lg:space-s-7'>
              {
                footerData.listPayment.map((item, index) => (
                  <List.Item classes='payment' key={index}>
                    <Link href={Enums.PATH.DEFAULT} target='_blank'>
                      <NextImage
                        src={item.srcImg}
                        alt={item.name}
                        width={item.width}
