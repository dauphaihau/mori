...
export default function DetailOrderDialog({ showDialog, setShowDialog, chargeId, initialValues }) {
  // const { purchasedProducts, isLoading, customer, mutate } = useDetailOrder(chargeId)
        ...
        <Box classes=''>
          ...
          <Row wrap='wrap' classes='mb-10 gap-y-4 gap-8'>
            ...
            <Box classes='border-dotted border-r-2 pr-4'>
              ...
              <Text b>
                {
                  initialValues?.metadata?.order_created_at ?
                    dayjs(Number(initialValues?.metadata.order_created_at)).format('LL')
                    : '-'
                }
              </Text>
            </Box>
            <Box classes='border-dotted border-r-2 pr-4'>
              ...
              <Text b>{initialValues?.billing_details?.email}</Text>
            </Box>
            <Box classes='border-dotted pr-4'>
              <Text transforms='uppercase' classes='text-xs font-medium text-gray-700'>
                total:
              </Text>
              <Text b>{formatDollarUSFromStripe(initialValues?.amount)}</Text>
            ...
            <Box classes='border-dotted pr-4'>
              ...
              <Text b>{initialValues?.payment_method_details?.type}</Text>
          ...
          <Box classes='mb-6'>
            ...
            <Box classes='p-4 border rounded'>
              <Text>{initialValues?.billing_details?.name}</Text>
              <Text>{initialValues?.billing_details?.address.country}</Text>
              <Text>{initialValues?.billing_details?.address.address1}</Text>
              <Text>{initialValues?.billing_details?.address.city}</Text>
              <Text>{initialValues?.billing_details?.address.state} {initialValues?.billing_details?.address.postal_code}</Text>
              <Text>{initialValues?.billing_details?.phone ?? '-'}</Text>
              <Text>{initialValues?.billing_details?.email ?? '-'}</Text>
